...
// Configuraciones por dificultad con más niveles y progresión
const difficultySettings = {
    easy: {
        ...
        enemyHealth: 1,
        enemySpawnRate: 1500,
        levelEnemies: [15, 20, 25, 30, 35, 40, 45, 50, 55, 60], // 10 niveles
        speedIncrement: 0.2, // Incremento de velocidad por nivel
        spawnRateDecrement: 50 // Reducción del tiempo de spawn por nivel
    },
    medium: {
        playerSpeed: 7,
        enemySpeed: 3,
        enemyHealth: 1,
        enemySpawnRate: 1000,
        levelEnemies: [20, 25, 30, 35, 40, 45, 50, 55, 60, 65], // 10 niveles
        speedIncrement: 0.3,
        spawnRateDecrement: 75
    },
    hard: {
        playerSpeed: 6,
        enemySpeed: 4,
        enemyHealth: 2,
        enemySpawnRate: 800,
        levelEnemies: [25, 30, 35, 40, 45, 50, 55, 60, 65, 70], // 10 niveles
        speedIncrement: 0.4,
...
class Player {
    ...
    update() {
        ...
        // Disparo con teclado o toque táctil (solo si es un toque, no arrastre)
        if ((keys[' '] || keys['ArrowUp'] || (touchControls.isTapping && !touchControls.isDragging)) && !this.isShooting) {
            this.isShooting = true;
            this.shoot();
        } else if (!keys[' '] && !keys['ArrowUp'] && !touchControls.isTapping) {
            this.isShooting = false;
        }
...
class Enemy {
    constructor(x, y, type = 0) {
        ...
        this.x = x;
        this.y = y;
        this.speed = difficultySettings[difficulty].enemySpeed + (level * difficultySettings[difficulty].speedIncrement);
        this.type = type;
        this.health = difficultySettings[difficulty].enemyHealth + Math.floor(level / 3); // Aumenta salud cada 3 niveles
...
function handleTouchStart(e) {
    ...
    touchControls.isTapping = true;
    
    touchControls.dragTimer = setTimeout(() => {
        touchControls.isDragging = true;
        touchControls.isTapping = false;
    }, 200);
}
...
function handleTouchMove(e) {
    e.preventDefault();
    if (!touchControls.isTapping && !touchControls.isDragging) return;
    ...
    if (Math.abs(touchControls.touchMoveX - touchControls.touchStartX) > 10 ||
        Math.abs(touchControls.touchMoveY - touchControls.touchStartY) > 10) {
        touchControls.isDragging = true;
        touchControls.isTapping = false;
        clearTimeout(touchControls.dragTimer);
    }
}
...
// Actualizar configuraciones del nivel
function updateLevelSettings() {
    const settings = difficultySettings[difficulty];
    const maxLevel = settings.levelEnemies.length;
    
    if (level > maxLevel) {
        levelEnemiesToDefeat = settings.levelEnemies[maxLevel - 1] + (level - maxLevel) * 5;
    } else {
        levelEnemiesToDefeat = settings.levelEnemies[level - 1] || 15;
    }
    
    enemySpawnRate = Math.max(300, settings.enemySpawnRate - (level * settings.spawnRateDecrement));
}
...
function checkCollisions() {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
            if (checkCollision(bullets[i], enemies[j])) {
                ...
                if (enemyDestroyed) {
                    ...
                    // 15% de probabilidad de generar un power-up
                    if (Math.random() < 0.15) {
    ...
    // Colisiones entre jugador y power-ups (CORREGIDO)
    for (let i = powerUps.length - 1; i >= 0; i--) {
...
// Verificar colisión entre dos objetos (CORREGIDO)
...
// Aplicar efecto de power-up (CORREGIDO)
